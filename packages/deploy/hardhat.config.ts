...
let contractMapping: {[k: string]: string} = {};
task(TASK_DEPLOY_RUN_DEPLOY, 'deploy run only')
...
extendEnvironment((hre) => {
  ...
  const defaultVersion = '@sandbox-smart-contracts/core';
  // sandbox-smart-contracts-0.0.30/src/solc_0.5/Test/FakeDai.sol:FakeDai
  // @sandbox-smart-contracts/core/src/solc_0.5/Test/FakeDai.sol:FakeDai
  hre.deployments = {
    ...hre.deployments,
    deploy: async (name: string, options: DeployOptions): Promise<DeployResult> => {
      ...
      if (!fqns.includes(contractName)) {
        const names = fqns.filter(x => x.endsWith(':' + contractName) && x.startsWith(defaultVersion));
        if (names.length === 0) {
      ...
      if (options.contract && typeof (options.contract) != 'string') {
        throw new Error('We don\'t support ArtifactData');
      }
      contractMapping[contractName] = options.contract || name;
      return originalDeploy(name, options);
    }
  };
});
export default addSourceFiles(config, ['@sandbox-smart-contracts/core', 'sandbox-smart-contracts']);
