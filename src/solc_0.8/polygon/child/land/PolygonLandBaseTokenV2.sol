...
abstract contract PolygonLandBaseTokenV2 is IPolygonLand, Initializable, ERC721BaseTokenV2 {
    ...
    function batchTransferFrom(
        ...
        bytes calldata data
    ) public override(ILandToken, ERC721BaseTokenV2) onlyAllowedOperator {
    ...
    function _regroup(
        ...
        if (size == 3) {
            _regroup3x3(from, to, x, y);
        } else if (size == 6) {
            _regroup6x6(from, to, x, y);
        } else if (size == 12) {
            _regroup12x12(from, to, x, y);
        } else if (size == 24) {
            _regroup24x24(from, to, x, y);
        }
    }
