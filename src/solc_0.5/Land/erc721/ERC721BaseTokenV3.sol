...
import {DefaultOperatorFilterer} from "../../contracts_common/opensea/DefaultOperatorFilterer.sol";

contract ERC721BaseTokenV3 is ERC721Events, SuperOperatorsV2, MetaTransactionReceiverV2, DefaultOperatorFilterer {
     ...
     * @param id The id of the token
    */
    function transferFrom(address from, address to, uint256 id) external onlyAllowedOperator {
     ...
     */
    function safeTransferFrom(address from, address to, uint256 id, bytes memory data) public onlyAllowedOperator {
        bool metaTx = _checkTransfer(from, to, id);
        _transferFrom(from, to, id);
        if (to.isContract()) {
            require(
     ...
     * @param id The id of the token
     */
    function safeTransferFrom(address from, address to, uint256 id) external onlyAllowedOperator {
     ...
     * @param data additional data
    */
    function batchTransferFrom(address from, address to, uint256[] calldata ids, bytes calldata data) external onlyAllowedOperator {
